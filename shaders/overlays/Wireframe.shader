	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 scene_view_position_at(float4 position_cs, float2 screenspace_offset)
			{
				...
				//if (depth_texture_value == 0) return 0;
			...
			inline float3 safe_mormalize(float3 v)
			{
				float dp3 = dot(v, v);
				//if (dp3 == 0) return 0;
				return v * rsqrt(dp3);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 scene_normal_m_m = safe_mormalize(cross(scene_pos_m_0 - scene_pos_0_0, scene_pos_0_m - scene_pos_0_0));
				float3 scene_normal_m_p = safe_mormalize(cross(scene_pos_0_p - scene_pos_0_0, scene_pos_m_0 - scene_pos_0_0));
				float3 scene_normal_p_m = safe_mormalize(cross(scene_pos_0_m - scene_pos_0_0, scene_pos_p_0 - scene_pos_0_0));
				float3 scene_normal_p_p = safe_mormalize(cross(scene_pos_p_0 - scene_pos_0_0, scene_pos_0_p - scene_pos_0_0));
